...
const ResumeCard = ({ badge, title, subTitle, des }) => {
  // Check if badge is empty (which was the case for "current" badge)
  const isOngoing = badge === "";

  return (
    <div className="w-full py-6 flex flex-col gap-2.5 border-b-[1px] border-b-zinc-800">
      {isOngoing ? (
        <h6 className="w-28 text-center text-sm py-[1px] text-white bg-green-600 border-[1px] border-green-500 rounded-sm">
          2022-Present
        </h6>
      ) : (
